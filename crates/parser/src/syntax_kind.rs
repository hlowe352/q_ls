...
impl rowan::Language for QLang {
    ...
    fn kind_from_raw(raw: rowan::SyntaxKind) -> SyntaxKind {
        assert!(
            ...
            "raw SyntaxKind value out of bounds"
        );
...
mod tests {
    ...
    fn maps_new_tokens() {
        assert_eq!(
            SyntaxKind::from_token(q_lexer::Token::Month),
            SyntaxKind::Month
        );
        assert_eq!(
            SyntaxKind::from_token(q_lexer::Token::Guid),
            SyntaxKind::Guid
        );
        assert_eq!(
            SyntaxKind::from_token(q_lexer::Token::Timespan),
            SyntaxKind::Timespan
        );
        assert_eq!(
            SyntaxKind::from_token(q_lexer::Token::Minute),
            SyntaxKind::Minute
        );
        assert_eq!(
            SyntaxKind::from_token(q_lexer::Token::Second),
            SyntaxKind::Second
        );
        assert_eq!(
            SyntaxKind::from_token(q_lexer::Token::Datetime),
            SyntaxKind::Datetime
        );
        assert_eq!(
            SyntaxKind::from_token(q_lexer::Token::ByteList),
            SyntaxKind::ByteList
        );
        assert_eq!(
            SyntaxKind::from_token(q_lexer::Token::CommentBlock),
            SyntaxKind::CommentBlock
        );
        assert_eq!(
            SyntaxKind::from_token(q_lexer::Token::DslLine),
            SyntaxKind::DslLine
        );
        assert_eq!(
            SyntaxKind::from_token(q_lexer::Token::FileSymbol),
